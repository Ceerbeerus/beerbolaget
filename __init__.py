...
REQUIREMENTS = ['beerbolaget==0.0.36b4']
...
async def async_setup(hass, config):
    ...
    latest_release = release(hass, conf_api_key, conf_ratebeer,
                             conf_store, conf_untappd)

    hass.bus.async_listen_once(EVENT_HOMEASSISTANT_START, latest_release.check_release())
...
class release():
    def __init__(self, hass, conf_api_key,
        ...
        self.beer_handler = beer_handler(self.api_key, conf_ratebeer,
                                         conf_store, conf_untappd)
